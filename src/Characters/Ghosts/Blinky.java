...
public class Blinky extends Creature {
    ...
    public Blinky(int tileSize, int startX, int startY, GameBoard gameBoard){
        ...
        characterLabel.setIcon(motionImages[1][0]);
        setOpaque(false);
        ...
        try {
            for (int i = 0; i < motionImages.length; ++i) {
                for (int j = 0; j < motionImages[i].length; ++j) {
                    String path = "src/Textures/Ghosts/Blinky/" + imageNames[imageIndex++];
    ...
    public void setRunning(boolean running) {
        ...
        if (running) {
            if (blinkyThread == null || !blinkyThread.isAlive()) {
                blinkyThread = new Thread(this);
                blinkyThread.start();
            }
        }
    }
