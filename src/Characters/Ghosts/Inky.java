...
public class Inky extends Creature {
    ...
    public Inky(int tileSize, int startX, int startY, GameBoard gameBoard){
        ...
        try {
            for (int i = 0; i < motionImages.length; ++i) {
                for (int j = 0; j < motionImages[i].length; ++j) {
                    String path = "src/Textures/Ghosts/Inky/" + imageNames[imageIndex++];
    ...
    public void updateAnimationFrame(int currentDirection){
    ...
    public void setRunning(boolean running) {
        ...
        if (running) {
            if (inkyThread == null || !inkyThread.isAlive()) {
                inkyThread = new Thread(this);
                inkyThread.start();
            }
        }
    }
