...
public class Pinky extends Creature {
    ...
    public Pinky(int tileSize, int startX, int startY, GameBoard gameBoard) {
        ...
        characterLabel.setIcon(motionImages[2][1]);
        setOpaque(false);
        ...
        try {
            for (int i = 0; i < motionImages.length; ++i) {
                for (int j = 0; j < motionImages[i].length; ++j) {
                    String path = "src/Textures/Ghosts/Pinky/" + imageNames[imageIndex++];
    ...
    public void setRunning(boolean running) {
        ...
        if (running) {
            if (pinkyThread == null || !pinkyThread.isAlive()) {
                pinkyThread = new Thread(this);
                pinkyThread.start();
            }
        }
    }
